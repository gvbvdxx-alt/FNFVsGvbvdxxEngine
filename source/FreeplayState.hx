...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if PRELOAD_ALL
		var leText:String = "Press Space To Listen To The Music, press control for a menu of game settings.";
		var size:Int = 16;
		#else
		var leText:String = "Press control for a menu of game settings.";
		var size:Int = 18;
		#end
		...
		else if (accepted)
		{
			...
			destroyFreeplayVocals();
					...
					if(sys.FileSystem.exists(Paths.inst(poop + '/'  + poop)) && !sys.FileSystem.exists(Paths.json(poop + '/' + poop))) { //the json doesn't exist, but the song files do, or you put a typo in the name
							...
							CoolUtil.coolError("Your song seems to not have an Inst.ogg, check the folder name in 'songs'!", "FNF VS Gvbvdxx Engine Anti-Crash Tool (Credit to the devlopers of JS Engine!)");
