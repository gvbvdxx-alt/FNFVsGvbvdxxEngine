...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'bedroom':
				...
				var stageFront:BGSprite = new BGSprite('backgrounds/bedroom/roombg', -1500, -1500, 0, 0);
				stageFront.setGraphicSize(Std.int(stageFront.width * 8));
				stageFront.updateHitbox();
				add(stageFront);
				
				
				var stageFronta:BGSprite = new BGSprite('backgrounds/bedroom/roomback', -800, 800, 1, 1);
				stageFronta.setGraphicSize(Std.int(stageFronta.width * 3));
				stageFronta.updateHitbox();
				add(stageFronta);
			
			
			
			case 'gvbvdxx-scratch-game':
			
				var stageFront:BGSprite = new BGSprite('scratchgamebg', -900, -900, 0.3, 0.3);
				stageFront.setGraphicSize(Std.int(stageFront.width * 8));
				stageFront.updateHitbox();
				add(stageFront);
				
				
				var stageFronta:BGSprite = new BGSprite('scratchgameclouds', -900, -900, 0.4, 0.4);
				stageFronta.setGraphicSize(Std.int(stageFronta.width * 6));
				stageFronta.updateHitbox();
				add(stageFronta);
				
				
				var stageFrontb:BGSprite = new BGSprite('scratchgametheme', -800, 650, 1, 1);
				stageFrontb.setGraphicSize(Std.int(stageFrontb.width * 3));
			...
			case 'labe':
				...
				var stageFront:BGSprite = new BGSprite('backgrounds/labe/BGlabe3', -1000, -700, 0.4, 0.4);
				stageFront.setGraphicSize(Std.int(stageFront.width * 7));
				stageFront.updateHitbox();
				add(stageFront);
				
				
				var stageFronta:BGSprite = new BGSprite('backgrounds/labe/BGlabe1', -1500, -1000, 1, 1);
				stageFronta.setGraphicSize(Std.int(stageFronta.width * 4.6));
				stageFronta.updateHitbox();
				add(stageFronta);
			...
			case 'minecraft':
				...
				var stageFront:BGSprite = new BGSprite('backgrounds/bg', -1300, -800, 1, 1);
			...
			case 'White':
				...
				var stageFront:BGSprite = new BGSprite('WhiteBG', -1500, -1500, 1, 1);
				stageFront.setGraphicSize(Std.int(stageFront.width * 8));
				stageFront.updateHitbox();
				add(stageFront);
			...
			case 'zomebiedude4712-background':
			
				var stageFront:BGSprite = new BGSprite('backgrounds/zombiedude4712/grey', -3000, -3000, 0, 0);
				stageFront.setGraphicSize(Std.int(stageFront.width * 10));
				stageFront.updateHitbox();
				add(stageFront);
				
				var stageFrontB:BGSprite = new BGSprite('backgrounds/zombiedude4712/blocks', -800, -400, 0.5,0.5);
				stageFrontB.setGraphicSize(Std.int(stageFrontB.width * 3));
				stageFrontB.updateHitbox();
				add(stageFrontB);
				
				var stageFrontC:BGSprite = new BGSprite('backgrounds/zombiedude4712/foreground', -800, 400, 1, 1);
				stageFrontC.setGraphicSize(Std.int(stageFrontC.width * 6));
				stageFrontC.updateHitbox();
				add(stageFrontC);
	...
	{
		scoreTxt.text = 'Current score: ' + songScore
		+ ' | Notes missed: ' + songMisses
		+ ' | Ranking: ' + ratingName
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			var isSus:Bool = note.isSustainNote; //Among us moment...................
			var leData:Int = Math.round(Math.abs(note.noteData));
			var leType:String = note.noteType;
			callOnLuas('goodNoteHit', [notes.members.indexOf(note), leData, leType, isSus]);
			
			if (leType == "nermalNote") {
				health -= 0.2;
				FlxG.sound.play(Paths.sound('RedNoteHit'), 1);
				camGame.shake(0.1, 0.2);
			}
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			{
				FlxG.camera.zoom += 0.019 * camZoomingMult;
				camHUD.zoom += 0.025 * camZoomingMult;
			}
